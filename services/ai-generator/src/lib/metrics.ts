/**
 * Zero-dependency Prometheus text-format metrics for ai-generator.
 * No prom-client required — emits valid Prometheus exposition format.
 */

const SERVICE = 'ai-generator';
...
export const httpRequests = defineCounter(
  'ai_generator_http_requests_total',
  'Total HTTP requests handled by ai-generator.',
);

export const aiRequests = defineCounter(
  'ai_generator_ai_requests_total',
  'Total AI generation requests by status.',
);

export const aiStageCalls = defineCounter(
  'ai_generator_stage_calls_total',
  'Total calls to each AI pipeline stage.',
);

// ── Gauges ────────────────────────────────────────────────────────────────────

export const activeRequests = defineGauge(
  'ai_generator_active_requests',
  'Number of in-flight AI generation requests.',
);
...
export function renderMetrics(): string {
  ...
  for (const def of registry) {
    ...
    if (def.entries.size === 0) {
      // Emit a zero baseline so the metric appears in Prometheus even before first event
