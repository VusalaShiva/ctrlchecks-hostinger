/**
 * Credential encryption — must use the same CREDENTIAL_ENCRYPTION_KEY as the worker
 * so that credentials written by one service can be decrypted by the other.
 *
 * Adapted from worker/src/credentials-system/secret-crypto.ts (identical algorithm).
 */
...
function encryptionKey(): Buffer {
  ...
  // 64-char hex → use as-is (raw 32-byte key)
...
export function decryptJson<T extends Record<string, unknown> = Record<string, unknown>>(
...
export function maskSecrets(value: unknown): unknown {
  ...
  return Object.fromEntries(
    Object.entries(value as Record<string, unknown>).map(([key, val]) => [
      key,
      /token|secret|password|key|authorization|credential/i.test(key)
        ? '[REDACTED]'
        : maskSecrets(val),
    ]),
  );
