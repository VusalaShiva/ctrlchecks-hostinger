/**
 * Zero-dependency Prometheus text-format metrics for execution-engine.
 * No prom-client required — emits valid Prometheus exposition format.
 */

const SERVICE = 'execution-engine';
...
export const httpRequests = defineCounter(
  ...
  'Total HTTP requests handled by execution-engine.',
);

export const engineJobs = defineCounter(
  'engine_jobs_total',
  'Total execution jobs processed by the engine.',
);

// ── Gauges ────────────────────────────────────────────────────────────────────

export const engineQueueDepth = defineGauge(
  'engine_queue_depth',
  'Current depth of the engine-owned execution queue.',
);

export const engineConcurrent = defineGauge(
  'engine_concurrent_jobs',
  'Number of jobs currently running in the engine consumer.',
);
