...
app.get('/health/live', (_req: Request, res: Response) => {
  res.json({ status: 'live', service: 'trigger-service', port: PORT });
});
...
app.get('/health', (_req: Request, res: Response) => {
  res.json({ status: 'ok', service: 'trigger-service', port: PORT });
});
...
app.use((err: Error, req: Request, res: Response, _next: NextFunction) => {
  ...
  console.error(`[${rid}] [trigger-service] Unhandled error:`, err);
  res.status(500).json({ error: 'Internal server error', ref: rid });
});

if (require.main === module) {
  app.listen(PORT, '127.0.0.1', () => {
    console.log(`[trigger-service] Listening on 127.0.0.1:${PORT} (internal only)`);
    console.log('[trigger-service] Phase 2 — webhook/form/chat handlers active; schedule stub (501)');
    console.log('[trigger-service] Public URLs remain on worker — do NOT change them');
  });

  const shutdown = (): void => {
    console.log('[trigger-service] Shutting down...');
