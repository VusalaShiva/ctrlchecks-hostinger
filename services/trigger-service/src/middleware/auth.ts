...
if (!process.env.TRIGGER_SERVICE_KEY) {
  console.warn(
    '[trigger-service] TRIGGER_SERVICE_KEY not set — ' +
...
export function requireAuth(req: Request, res: Response, next: NextFunction): void {
  ...
  // Service-to-service path: worker → trigger-service
  if (serviceKey && req.headers['x-service-key'] === serviceKey) {
    return next();
  }

  // User-facing path: browser → trigger-service (Cognito JWT)
