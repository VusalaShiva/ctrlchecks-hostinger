...
if (!process.env.WORKFLOW_CRUD_SERVICE_KEY) {
  console.warn(
    '[workflow-crud-service] WORKFLOW_CRUD_SERVICE_KEY not set — ' +
...
export function requireAuth(req: Request, res: Response, next: NextFunction): void {
  ...
  // Service-to-service path: worker → workflow-crud-service
  if (serviceKey && req.headers['x-service-key'] === serviceKey) {
    return next();
  }

  // User-facing path: browser → workflow-crud-service (Cognito JWT)
