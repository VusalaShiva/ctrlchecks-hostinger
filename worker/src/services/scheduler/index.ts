...
class SchedulerService {
   ...
   * Public so it can be delegated to trigger-service and unit-tested.
   */
  async executeScheduledWorkflow(workflowId: string) {
    try {
      ...
      if (shouldUseTriggerService(workflowId)) {
        try {
          ...
          if (remote) {
            console.log(`[scheduler] trigger-service dispatched ${workflowId}: exec=${remote.executionId}`);
            return;
          }
        } catch {
          // non-fatal — fall through to local fetch below
        }
        console.warn(`[scheduler] trigger-service returned null for ${workflowId} — local fallback`);
      }

      // Local path: POST directly to execute-workflow
